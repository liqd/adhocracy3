...
javascript developement dependencies
````````````````````````````````````
Check for outdated packages::

    bin/yarn outdated

Then update development package.json generated by src/adhocracy_frontend/checkcode_and_compile.cfg
Rerun buildout and commit changes to yarn.lock and checkcode_and_compile.cfg

javascript production dependencies
``````````````````````````````````
Check for outdated packages::

    bin/yarn outdated --modules-folder src/<current_customm_frontend_package>/build/lib

Then update prduction package.json generated by src/adhocracy_frontend/checkcode_and_compile.cfg
