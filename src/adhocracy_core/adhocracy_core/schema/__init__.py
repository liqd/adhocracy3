...
class ResourceObjectType(SchemaType):
    ...
    def _deserialize_location_or_url(self, node, value):
        if value.startswith('/'):
            ...
            root_url = request.resource_url(request.root, route_name=API_ROUTE_NAME)
...
def get_choices_by_interface(interface: IInterface,
    ...
    choices = [(request.resource_url(r, route_name=API_ROUTE_NAME), resource_path(r)) for r in resources]
    return choices
