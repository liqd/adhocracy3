...
from substanced.principal import _gen_random_token
...
class IPrincipalsPool(IPool):
    ...
    If the object is created via :meth:`substanced.content.ContentRegistry.create`,
    it will contain three subobjects:

      users: an instance of the content type `Users`
...
class PrincipalsPool(Pool):
    ...
    def add_group(self, name: str, registry=None) -> object:
        ...
        registry = registry or get_current_registry()
        group = registry.content.create('Group')  # FIXME don`t override users
